...
def test_all_test_mols_rank_themselves_top(test_mol_smiles):
    ...
    assert all(
        [t[0] == GetFepopStatusCode.SUCCESS for t in fepops_dict.values()]
    ), "Could not generate valid fepops for all molecules within the test set"
    scores = {}
    ...
    sorted_results = {
        query: {
            k: v
            for k, v in sorted(
                scores[query].items(), key=lambda item: item[1], reverse=True
            )
        }
