...
class Fepops():
  ...
  def __init__(self):
    ...
    self.acceptor_mol_from_smarts = Chem.MolFromSmarts("[!$([#6,F,Cl,Br,I,o,s,nX3,#7v5,#15v5,#16v4,#16v6,*+1,*+2,*+3])]")
    self.rotatable_bond_from_smarts = Chem.MolFromSmarts('[!$(*#*)&!D1]-&!@[!$(*#*)&!D1]')
  ...
  def _get_k_medoids(self, input_x:np.array, k:int=7) -> np.array:
    ...
    medoids = input_x[np.random.choice(np.arange(input_x.shape[0]), size=k, replace=False), :]
    
    while (point_to_centroid_map != point_to_centroid_map_prev).any():
      point_to_centroid_map_prev = point_to_centroid_map
      point_to_centroid_map = np.argmin(np.square(cdist(input_x, medoids)), axis=1)
      for i in range(k):
  ...
  def _calculate_atomic_logPs(self, mol:Chem.rdchem.Mol) -> dict:
    ...
    """
    return {atom.GetIdx():float(contribution[0]) for atom, contribution in zip(mol.GetAtoms(), Crippen.rdMolDescriptors._CalcCrippenContribs(mol))}
  ...
  def _calculate_partial_charges(self, mol:Chem.rdchem.Mol) -> dict:
    ...
    return {atom.GetIdx():float(atom.GetProp("_GasteigerCharge")) for atom in mol.GetAtoms()}
  
  def _sum_of_atomic_features_by_centroids(self, feature_dict:dict, centroid_atom_idx:np.array) -> int:
  ...
  def _get_flanking_atoms(self, bonds:Chem.rdchem._ROBondSeq, atom_1_idx:int, atom_2_idx:int) -> tuple:
    ...
    for bond in bonds:
      ...
      if atom_1_idx in bond_indexes: # Atom 1 in bond indexes
        if atom_2_idx in bond_indexes: continue
  ...
  def _sample_bond_states(self, n_rot:int) -> list:
    ...
    rotation_list=set(tuple(random.choice(range(4)) for _ in range(n_rot)) for counter in range(1024))
  ...
  def generate_conformers(self, mol:Chem.rdchem.Mol) -> list:
    ...
    starting_angles = (rdMolTransforms.GetDihedralDeg(original_conformer, *dihedral_atoms) for dihedral_atoms in dihedrals)
    bond_states = self._sample_bond_states(len(dihedrals))
      
    new_conf_mol_list=[]
    for bond_state in bond_states:
      self._generate_conf(original_conformer, dihedrals, starting_angles, bond_state)
  ...
  def _generate_conf(self, conformer:Chem.rdchem.Conformer, dihedrals:tuple, starting_angles:tuple, bond_state:tuple) -> None:
  ...
  def get_centroid_pharmacophoric_features(self, mol:Chem.rdchem.Mol, num_centroids:int=4) -> np.array:
    ...
    hb_acceptors = set(i[0] for i in mol.GetSubstructMatches(self.acceptor_mol_from_smarts))
    hb_donors = set(i[0] for i in mol.GetSubstructMatches(self.donor_mol_from_smarts))
    ...
    for centroid in range(num_centroids):
      ...
      if len(hb_donors.intersection(set(centroid_atomic_id))) > 0: hbd=1
      pharmacophore_features_arr = np.append(pharmacophore_features_arr, [sum_of_logP, sum_of_charge, hba, hbd])

    pharmacophore_features_arr = np.append(pharmacophore_features_arr, centroid_dist)
  ...
  def get_fepops(self, smiles_string:str) -> np.array:
    ...
    for index, each_mol in enumerate(each_mol_with_all_confs_list):
      ...
      if index == 0: 
        ...
        continue
      pharmacophore_feature_all_confs = np.vstack((pharmacophore_feature_all_confs, pharmacophore_feature))
    return self._get_k_medoids(pharmacophore_feature_all_confs, 7)

  @staticmethod
  def cal_similarity(fepops_features_1:np.array, fepops_features_2:np.array) -> None:
    ...
    """
    print(np.max(cdist(fepops_features_1, fepops_features_2, metric=_score)))

def _score(x1:np.array, x2:np.array) -> float:
...
if __name__ == "__main__":
    ...
    f1_parser = subparsers.add_parser("get_fepops", help="Generate the FEPOPS descriptors from an input SMILES string")
    f1_parser.add_argument("-ismi", "--input_smiles", help="An input SMILES string of a molecule", dest= "insmile", required=True)

    f2_parser = subparsers.add_parser("cal_sim", help="Calculate FEPOPS similarity between two molecules")
    f2_parser.add_argument("-ismi1", "--input_smiles_1", help="An input SMILES string of the first molecule", dest= "insmiles1", required=True)
    f2_parser.add_argument("-ismi2", "--input_smiles_2", help="An input SMILES string of the second molecule", dest= "insmiles2", required=True)

    args = parser.parse_args()
    f = Fepops()
    if args.subcmd == "get_fepops":
      #fepops_features = f.get_fepops("O=C1OC2=CC3(C)C(CC4OC(=O)C(OC(=O)C)C5C6(OCC45C3C(O)C6O)C(=O)OC)C(C2=C1)C")
    ...
    if args.subcmd == "cal_sim":
      ...
      f.cal_similarity(fepops_features_1, fepops_features_2)