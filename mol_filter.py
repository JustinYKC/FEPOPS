...
class Filter():
  ...
  def __init__(self, min_atoms:int=4, max_rings:int=9, max_n_rot:int=40) -> None:
  ...
  def atom_num(self, mol:Chem.rdchem.Mol, cutoff:int=4) -> Chem.rdchem.Mol or None:
    ...
    if atom_num > cutoff: return mol
    else: return None

  def ring_num(self, mol:Chem.rdchem.Mol, cutoff:int=9) -> Chem.rdchem.Mol or None:
    ...
    if ring_num < cutoff: return mol
    else: return None

  def rotation_bond_num(self, mol:Chem.rdchem.Mol, cutoff:int=40) -> Chem.rdchem.Mol or None:
    ...
    if rotation_bond_num < cutoff: return mol
    else: return None
...
if __name__ == "__main__":
  dataset_raw = pd.read_csv("./COCONUT_DB.smi", sep=" ", header=0, names= ["SMILES", "ID"])
  print (dataset_raw)
  ...
  dataset_vali.to_csv('./COCONUT_after_filter.smi', columns=["Std_SMILES","ID"], sep="\t", index=False, header=False)